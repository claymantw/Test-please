...
const shareStats = () => { 
  ...
  let shareText = `My Minesweeper stats:\n\n💣 ${minisweeperState.stats.games} game${gamesPlayedSuffix} played\n🏆 ${minisweeperState.stats.wins} game${gamesWonSuffix} won\n\nThat's a 📊 ${(Math.round(minisweeperState.stats.wins *100) / minisweeperState.stats.games).toFixed(2)}% success rate!`; 
  if(minisweeperState.stats.fastest) { 
    shareText += `\n\nMy fastest win: ${parseInt(minisweeperState.stats.fastest/1000)}.${parseInt(minisweeperState.stats.fastest%1000)} seconds!`; 
  }
  shareText+= `\n\nThink you can do better? Play here 👇`;
  farcasterSDK.actions.openUrl(`https://warpcast.com/~/compose?text=${encodeURIComponent(shareText)}&embeds[]=${appURL}`); 
  return false; 
}
